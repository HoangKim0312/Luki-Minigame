...
const SESSION_KEY = "anigame-session";
const SESSION_EVENT = "anigame-auth-change";
...
export async function authApi<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  const send = (current: Session | null) => fetch(`${getBackendUrl()}${path}`, {
    ...options,
    headers: {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = useCallback((next: Session) => { publishSession(next); setSession(next); setStatus("authenticated"); }, []);
  const signOut = useCallback(() => { publishSession(null); setSession(null); setStatus("anonymous"); }, []);
